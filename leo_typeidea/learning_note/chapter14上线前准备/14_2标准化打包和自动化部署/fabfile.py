...
from fabric.decorators import tasks

env.roledefs = {
    'myserver': ['leo@127.0.0.1'],
}
env.PROJECT_NAME = 'leo_typeidea'
env.SETTINGS_BASE = 'leo_typeidea/leo_typeidea/settings/base.py'

env.DEPLOY_PATH = '/home/leo/venvs/leo_typeidea-env'
...
def _ensure_virtualenv():
    ...
    return ('python3.6 -m venv %s' % env.DEPLOY_PATH)
...
def _reload_supervisord(deploy_path, profile):
    ...
    upload_template(filename,destination,context,use_jinja=True, template_dir=template_dir)
    with settings(warn_only=True):
        result = run('supervisorctl -c %s/supervisord.conf shutdown' % deploy_path)
        if result:
            run('supervisorctl -c %s/supervisord.conf' % deploy_path)
...
def deploy(version,profile):
    ...
    with prefix('source %s' % env.VENV_ACTIVATE):
        ...
        run('echo yes |grep %s/bin/manage.py collectstatic' % env.DEPLOY_PATH)